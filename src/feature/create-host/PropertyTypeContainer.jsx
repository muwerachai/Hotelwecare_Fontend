...
function PropertyTypeContainer() {
  let dataMock = new Array(8).fill({
    propertyName: "Camper/RV",
    propertyDetail:
      "A motorhome or camping trailer that's half-home and half-vehicle."
  });
  return (
    <div className="flex flex-row min-h-screen">
      ...
      <div className="basis-2/4 bg-white flex flex-col justify-between">
        ...
        <div className="px-5 pb-10">
          {dataMock.map((item, keys) => {
            return (
              <div
                key={keys}
                className="py-5 border-2 text-start bg-white border-slate-400 flex flex-col flex-nowrap  rounded-lg mt-5 mx-10 hover:border-black"
              >
                <p className="pl-6 text-start font-medium">
                  {item.propertyName}
                </p>
                <p className="pl-6 font-light">{item.propertyDetail}</p>
              </div>
            );
          })}
        </div>
        <div className="sticky bottom-0 right-0 z-50 flex justify-between border-t bg-white border-t-slate-200 py-4 pr-12">
          ...
          <Link
            to={"/create-host/privacy-type/123"}
